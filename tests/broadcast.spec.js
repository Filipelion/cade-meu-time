...
test.describe("Broadcast label", () => {
  ...
  test("renders TV icon alongside channel name", async ({
    page,
    extensionId,
  }) => {
    await setup(page, extensionId);
  ...
  test("broadcast text turns yellow on hover — light mode", async ({
    page,
    extensionId,
  }) => {
    await setup(page, extensionId);
    ...
    expect(await textColor(page.locator(".game-broadcast").first())).toBe(
      YELLOW,
    );
  });

  test("broadcast text turns yellow on hover — dark mode", async ({
    page,
    extensionId,
  }) => {
    await setup(page, extensionId);
