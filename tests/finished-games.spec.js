...
test.describe("Jogos Encerrados", () => {
  test("panel is hidden and upcoming games are visible by default", async ({
    page,
    extensionId,
  }) => {
  ...
  test("clicking button shows finished games and hides upcoming games", async ({
    page,
    extensionId,
  }) => {
    await setup(page, extensionId);
  ...
  test("button gets active class when panel is open", async ({
    page,
    extensionId,
  }) => {
    await setup(page, extensionId);
  ...
  test("active button has red background and yellow text", async ({
    page,
    extensionId,
  }) => {
    await setup(page, extensionId);
    ...
    expect(await textColor(page.locator(".game-card-finished-text"))).toBe(
      YELLOW,
    );
  });

  test("renders the correct number of finished game cards", async ({
    page,
    extensionId,
  }) => {
    await setup(page, extensionId);
  ...
  test("cards display score", async ({ page, extensionId }) => {
    ...
    await page.waitForSelector(".game--finished");
    await expect(
      page.locator(".game--finished .score-text").first(),
    ).toHaveText("2 - 1");
  });
  ...
  test("cards display date", async ({ page, extensionId }) => {
    ...
    await page.waitForSelector(".game--finished");
    await expect(
      page.locator(".game--finished .finished-date").first(),
    ).toHaveText("SÁB, 18/04");
  });

  test('cards have "Veja os detalhes" tooltip', async ({
    page,
    extensionId,
  }) => {
    await setup(page, extensionId);
    await page.click("#btn-finished-games");
    await page.waitForSelector(".game--finished");
    await expect(page.locator(".game--finished").first()).toHaveAttribute(
      "title",
      "Veja os detalhes",
    );
  });

  test("cards link to placardefutebol in a new tab", async ({
    page,
    extensionId,
  }) => {
    await setup(page, extensionId);
  ...
  test("second click hides panel and restores upcoming games", async ({
    page,
    extensionId,
  }) => {
    await setup(page, extensionId);
  ...
  test("button loses active class when panel is closed", async ({
    page,
    extensionId,
  }) => {
    await setup(page, extensionId);
