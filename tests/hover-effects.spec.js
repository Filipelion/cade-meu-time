...
for (const [label, applyMode] of [
  ...
  test.describe(`Hover effects — ${label}`, () => {
    test("game card turns red with yellow text on hover", async ({
      page,
      extensionId,
    }) => {
      ...
      expect(await textColor(page.locator(".game-league").first())).toBe(
        YELLOW,
      );
      expect(await textColor(page.locator(".game-date").first())).toBe(YELLOW);
    });

    test("game venue text turns yellow on hover", async ({
      page,
      extensionId,
    }) => {
      await setup(page, extensionId);
    ...
    test("news card turns red with yellow title on hover", async ({
      page,
      extensionId,
    }) => {
      await setup(page, extensionId);
    ...
    test("video card turns red with yellow title on hover", async ({
      page,
      extensionId,
    }) => {
      await setup(page, extensionId);
      ...
      await card.hover();
      expect(await bgColor(card)).toBe(RED);
      expect(await textColor(page.locator(".video-title").first())).toBe(
        YELLOW,
      );
    });
  });
