...
export function isFinishedCacheValid() {
  ...
  return (
    !!lastFetched && Date.now() - Number(lastFetched) < FINISHED_CACHE_TTL_MS
  );
}
...
export function isTicketsCacheValid() {
  ...
  return (
    !!lastFetched && Date.now() - Number(lastFetched) < TICKETS_CACHE_TTL_MS
  );
}
...
export function isSociosCacheValid() {
  ...
  return (
    !!lastFetched && Date.now() - Number(lastFetched) < SOCIOS_CACHE_TTL_MS
  );
}
