const LAST_GAMES_URL = 'https://www.placardefutebol.com.br/time/sport/ultimos-jogos';

function extractCardMeta(card) {
  return {
    team_home: card.querySelector('.match__lg_card--ht-name.text')?.textContent.trim() ?? '',
    team_away: card.querySelector('.match__lg_card--at-name.text')?.textContent.trim() ?? '',
    img_src_home: card.querySelector('.match__lg_card--ht-logo img')?.getAttribute('src') ?? '',
    img_src_away: card.querySelector('.match__lg_card--at-logo img')?.getAttribute('src') ?? '',
    campeonato: card.querySelector('.match__lg_card--league')?.textContent.trim() ?? '',
...
async function fetchReloadData(link) {
  if (!link) return null;
  const slug = link.replace(/^https?:\/\/[^/]+\//, '').replace(/^\//, '').replace(/\.html$/, '');
  try {
...
export async function fetchLiveGameFromLastGames() {
  try {
    ...
    for (const card of cards) {
      ...
      return { isLive: true, ...meta, score_home: reload.score_home, score_away: reload.score_away, minute: reload.minute };
    }
    return null;
  } catch {
    return null;
...
export function parseReloadFromHTML(html) {
  ...
  const isLive = (statusEl?.classList.contains('badge-success') ?? false) || minute === 'Intervalo';
  return {
    isLive,
